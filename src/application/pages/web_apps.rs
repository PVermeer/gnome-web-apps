...
impl WebAppsPage {
    ...
    fn get_isolation_path(app: &Rc<App>, app_id: &str) -> Result<PathBuf> {
        let profiles_path = app.dirs.create_data_directory("profiles")?;
        let path = profiles_path.join(app_id);
        Ok(path)
    }

    fn get_optional(key: &str, text: &str) -> Option<String> {
    ...
    fn save(app: &Rc<App>, desktop_file: &RefCell<DesktopEntry>) -> Result<()> {
        if let Err(error) = (|| -> Result<()> {
            ...
            if let Some(value) = Self::get_optional(isolate_key, &d_str) {
                let path = Self::get_isolation_path(app, &entries.id)?;
